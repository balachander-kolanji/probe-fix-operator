...
```bash
docker build -t registry.kci.rocks/devops/probe-timeout-operator:latest .
docker login registry.kci.rocks -u <username> -p <token>
docker push registry.kci.rocks/devops/probe-timeout-operator:latest
...
```
cd existing_repo
git remote add origin https://git.kci.rocks/DevOps/probe-timeout-operator.git
git branch -M master
...
- [ ] [Set up project integrations](https://git.kci.rocks/DevOps/probe-timeout-operator/-/settings/integrations)
